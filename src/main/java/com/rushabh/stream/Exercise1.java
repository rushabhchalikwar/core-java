...
public class Exercise1 {
  public static void main(String[] args) {
    ...
    Map<String, List<Employee>> deptMap =
        employees.stream().collect(Collectors.groupingBy(emp -> emp.getDepartNames()));

    // Find the maximum age among the employees using reduce
    int maxAge =
        employees.stream().map(employee -> employee.getAge()).reduce(0, (a, b) -> a > b ? a : b);

    // Get a list of distinct department names from the employees
    List<String> deptNames =
        employees.stream().map(employee -> employee.getDepartNames()).distinct().toList();
